...
if ${MOVIES_STATUS:-false}; then
    if [[ -d "${MOVIES_DIR}" ]]; then
        MOVIES_LIST=$(get_movies_list "${MOVIES_DIR}")
    else echo "This is not a directiory!"
        exit 1
    fi
else echo "You need to use -d!"
    exit 1
fi
...
if [[ "${FILE_4_SAVING_RESULTS:-}" == "" ]]; then
    print_movies "${MOVIES_LIST}" "${OUTPUT_FORMAT:-raw}" | tee "results.txt"
elif [[ "${FILE_4_SAVING_RESULTS:-}" == *.txt ]]; then
    print_movies "${MOVIES_LIST}" "${OUTPUT_FORMAT:-raw}" | tee "${FILE_4_SAVING_RESULTS}"
else 
    NAME="${FILE_4_SAVING_RESULTS}"
    NAME+=".txt"
    print_movies "${MOVIES_LIST}" "${OUTPUT_FORMAT:-raw}" | tee "${NAME}"
fi
